...
fsrc = open(str(sys.argv[1]), 'r', encoding='utf-8')

lines = []
lines_bin = []
names = []

instructions = ['add', 'sub', 'mul', 'div', 'mod', 'load', 'mov', 'store', 'goto', 'jz', 'jn', 'halt', 'wb', 'ww']
...
def encode_instruction(inst, ops):
   if inst in ['add', 'sub', 'mul', 'div', 'mod', 'load', 'mov', 'store', 'jz', 'jn']:
...
def resolve_names():
   ...
   for line in lines_bin:
      for i in range(0, len(line)):
         if is_name(line[i]):
            if line[i-1] in word_opcodes:
               line[i] = get_name_byte(line[i]) // 4
            else:
               line[i] = get_name_byte(line[i])

for line in fsrc:
   linha_limpa = line.split(';')[0]
   tokens = linha_limpa.replace('\n','').replace(',','').lower().split(" ")
   i = 0
   while i < len(tokens):
      if tokens[i] == '':
         tokens.pop(i)
